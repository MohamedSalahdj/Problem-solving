...
class Cipher(object):
    ...
    def _translate(self, s, source_map, target_map):
        return "".join(
            target_map[source_map.find(char)] if char in source_map else char
            for char in s
        )
    
    def encode(self, s):
        return self._translate(s, self.map1, self.map2)
    
    def decode(self, s):
        return self._translate(s, self.map2, self.map1)