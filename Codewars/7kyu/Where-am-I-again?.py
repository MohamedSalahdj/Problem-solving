"""
Problem description
    Mr. Strolly lives in a fancy neighborhood called Gridsville. Once a day he likes to take a walk. Since his neighborhood is a grid, 
    he can either walk in the directions of North(n), East(e), South(s) or West(w). 
    Mr. Strolly's problem is that he often goes on long walks and can't remember exactly where he is. 
    Even when he can remember the path he took, when he is ready to go home, he wants to take the shortest path.
...
class Walker:    
    ...
    def walk(self, path):
        for p in path:
            self.xcord += {'e': 1, 'w': -1}.get(p, 0)
            self.ycord += {'n': 1, 's': -1}.get(p, 0)
